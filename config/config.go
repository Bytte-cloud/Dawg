...
const DefaultLocation = "/etc/dawg/config.yml"
...
type SystemConfiguration struct {
	// The root directory where all of the Dawg data is stored at.
	RootDirectory string `default:"/var/lib/dawg" json:"-" yaml:"root_directory"`

	// Directory where logs for server installations and other wings events are logged.
	LogDirectory string `default:"/var/log/dawg" json:"-" yaml:"log_directory"`

	// Directory where the server data is stored at.
	Data string `default:"/var/lib/dawg/volumes" json:"-" yaml:"data"`

	// Directory where server archives for transferring will be stored.
	ArchiveDirectory string `default:"/var/lib/dawg/archives" json:"-" yaml:"archive_directory"`

	// Directory where local backups will be stored on the machine.
	BackupDirectory string `default:"/var/lib/dawg/backups" json:"-" yaml:"backup_directory"`

	// TmpDirectory specifies where temporary files for Dawg installation processes
	// should be created. This supports environments running docker-in-docker.
	TmpDirectory string `default:"/tmp/dawg" json:"-" yaml:"tmp_directory"`

	// The user that should own all of the server files, and be used for containers.
	Username string `default:"dawg" yaml:"username"`
...
type QemuConfiguration struct {
	...
	DataDirectory string `default:"/var/lib/dawg/vms" json:"-" yaml:"data_directory"`
	...
	TemplateDirectory string `default:"/var/lib/dawg/vm-templates" json:"-" yaml:"template_directory"`
...
type Configuration struct {
	...
	AppName string `default:"Dawg" json:"app_name" yaml:"app_name"`
...
// EnsureDawgUser ensures that the Dawg core user exists on the
...
func EnsureDawgUser() error {
	...
	if sysName == "distroless" {
		_config.System.Username = system.FirstNotEmpty(os.Getenv("WINGS_USERNAME"), "dawg")
	...
	log.WithField("username", _config.System.Username).Info("checking for dawg system user")
